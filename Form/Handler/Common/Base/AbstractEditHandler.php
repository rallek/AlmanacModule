...
namespace RK\AlmanacModule\Form\Handler\Common\Base;
...
use RK\AlmanacModule\Helper\HookHelper;
use RK\AlmanacModule\Helper\ModelHelper;
...
abstract class AbstractEditHandler
{
    ...
    public function processForm(array $templateParameters = [])
    {
        ...
        // default to referer
        $refererSessionVar = 'rkalmanacmodule' . $this->objectTypeCapital . 'Referer';
        ...
        $this->permissionComponent = 'RKAlmanacModule:' . $this->objectTypeCapital . ':';
        ...
        if ($this->templateParameters['mode'] == 'edit') {
            ...
            if (null !== $entity) {
                if (true === $this->hasPageLockSupport && $this->kernel->isBundle('ZikulaPageLockModule') && null !== $this->lockingApi) {
                    ...
                    $lockName = 'RKAlmanacModule' . $this->objectTypeCapital . $entity->getKey();
            ...
            $permissionLevel = in_array($this->objectType, ['date']) ? ACCESS_COMMENT : ACCESS_EDIT;
        ...
        if (false === $actions || !is_array($actions)) {
            ...
            $logArgs = ['app' => 'RKAlmanacModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $entity->getKey()];
    ...
    public function handleCommand(array $args = [])
    {
        ...
        if ($entity->supportsHookSubscribers()) {
            ...
            if ($action != 'delete') {
                ...
                $routeUrl = new RouteUrl('rkalmanacmodule_' . $this->objectTypeLower . '_display', $urlArgs);
            }
    ...
    protected function addDefaultMessage(array $args = [], $success = false)
    {
        ...
        $logArgs = ['app' => 'RKAlmanacModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $this->objectType, 'id' => $this->entityRef->getKey()];
        if (true === $success) {
    ...
    public function fetchInputData()
    {
        ...
        if (isset($this->form['additionalNotificationRemarks']) && $this->form['additionalNotificationRemarks']->getData() != '') {
            $this->request->getSession()->set('RKAlmanacModuleAdditionalNotificationRemarks', $this->form['additionalNotificationRemarks']->getData());
        }
    ...
    protected function prepareWorkflowAdditions($enterprise = false)
    {
        ...
        $groupApplicationArgs = [
            'user' => $currentUserId,
            'group' => $this->variableApi->get('RKAlmanacModule', 'moderationGroupFor' . $this->objectTypeCapital, GroupsConstant::GROUP_ID_ADMIN)
        ];
        ...
        if (true === $enterprise) {
            $groupApplicationArgs = [
                'user' => $currentUserId,
                'group' => $this->variableApi->get('RKAlmanacModule', 'superModerationGroupFor' . $this->objectTypeCapital, GroupsConstant::GROUP_ID_ADMIN)
            ];
