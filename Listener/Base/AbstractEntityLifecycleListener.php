...
use RK\AlmanacModule\AlmanacEvents;
use RK\AlmanacModule\Event\FilterDateEvent;
...
abstract class AbstractEntityLifecycleListener implements EventSubscriber, ContainerAwareInterface
{
    ...
    public function preRemove(LifecycleEventArgs $args)
    {
        ...
        $this->eventDispatcher->dispatch(constant('\\RK\\AlmanacModule\\AlmanacEvents::' . strtoupper($entity->get_objectType()) . '_PRE_REMOVE'), $event);
    ...
    public function postRemove(LifecycleEventArgs $args)
    {
        ...
        if (count($uploadFields) > 0) {
            $uploadHelper = $this->container->get('rk_almanac_module.upload_helper');
        ...
        $logArgs = ['app' => 'RKAlmanacModule', 'user' => $currentUserApi->get('uname'), 'entity' => $objectType, 'id' => $entity->getKey()];
        $this->logger->debug('{app}: User {user} removed the {entity} with id {id}.', $logArgs);
        
        // create the filter event and dispatch it
        $event = $this->createFilterEvent($entity);
        $this->eventDispatcher->dispatch(constant('\\RK\\AlmanacModule\\AlmanacEvents::' . strtoupper($objectType) . '_POST_REMOVE'), $event);
    }
    ...
    public function prePersist(LifecycleEventArgs $args)
    {
        ...
        // create the filter event and dispatch it
        $event = $this->createFilterEvent($entity);
        $this->eventDispatcher->dispatch(constant('\\RK\\AlmanacModule\\AlmanacEvents::' . strtoupper($entity->get_objectType()) . '_PRE_PERSIST'), $event);
        if ($event->isPropagationStopped()) {
            return false;
        }
    }
    ...
    public function postPersist(LifecycleEventArgs $args)
    {
        ...
        $currentUserApi = $this->container->get('zikula_users_module.current_user');
        $logArgs = ['app' => 'RKAlmanacModule', 'user' => $currentUserApi->get('uname'), 'entity' => $entity->get_objectType(), 'id' => $entity->getKey()];
        $this->logger->debug('{app}: User {user} created the {entity} with id {id}.', $logArgs);
        
        // create the filter event and dispatch it
        $event = $this->createFilterEvent($entity);
        $this->eventDispatcher->dispatch(constant('\\RK\\AlmanacModule\\AlmanacEvents::' . strtoupper($entity->get_objectType()) . '_POST_PERSIST'), $event);
    }
    ...
    public function preUpdate(PreUpdateEventArgs $args)
    {
        ...
        // create the filter event and dispatch it
        $event = $this->createFilterEvent($entity);
        $this->eventDispatcher->dispatch(constant('\\RK\\AlmanacModule\\AlmanacEvents::' . strtoupper($entity->get_objectType()) . '_PRE_UPDATE'), $event);
        if ($event->isPropagationStopped()) {
            return false;
        }
    }
    ...
    public function postUpdate(LifecycleEventArgs $args)
    {
        ...
        // create the filter event and dispatch it
        $event = $this->createFilterEvent($entity);
        $this->eventDispatcher->dispatch(constant('\\RK\\AlmanacModule\\AlmanacEvents::' . strtoupper($entity->get_objectType()) . '_POST_UPDATE'), $event);
    }
    ...
    public function postLoad(LifecycleEventArgs $args)
    {
        ...
        // create the filter event and dispatch it
        $event = $this->createFilterEvent($entity);
        $this->eventDispatcher->dispatch(constant('\\RK\\AlmanacModule\\AlmanacEvents::' . strtoupper($entity->get_objectType()) . '_POST_LOAD'), $event);
    }
    ...
    protected function isEntityManagedByThisBundle($entity)
    {
        ...
        return ($entityClassParts[0] == 'RK' && $entityClassParts[1] == 'AlmanacModule');
    }
    ...
    {
        $filterEventClass = '\\RK\\AlmanacModule\\Event\\Filter' . ucfirst($entity->get_objectType()) . 'Event';
