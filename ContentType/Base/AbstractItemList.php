...
namespace RK\AlmanacModule\ContentType\Base;
...
abstract class AbstractItemList extends \Content_AbstractContentType implements ContainerAwareInterface
{
    ...
    {
        return $this->container->get('translator.default')->__('RKAlmanacModule list view');
    }
    ...
    {
        return $this->container->get('translator.default')->__('Display list of RKAlmanacModule objects.');
    }
    ...
    {
        $controllerHelper = $this->container->get('rk_almanac_module.controller_helper');
        ...
        $featureActivationHelper = $this->container->get('rk_almanac_module.feature_activation_helper');
        if ($featureActivationHelper->isEnabled(FeatureActivationHelper::CATEGORIES, $this->objectType)) {
            $this->categorisableObjectTypes = ['date'];
            $categoryHelper = $this->container->get('rk_almanac_module.category_helper');
    
            // fetch category properties
            $this->catRegistries = [];
            $this->catProperties = [];
            if (in_array($this->objectType, $this->categorisableObjectTypes)) {
                $entityFactory = $this->container->get('rk_almanac_module.entity_factory');
    ...
    {
        $repository = $this->container->get('rk_almanac_module.entity_factory')->getRepository($this->objectType);
        $permissionApi = $this->container->get('zikula_permissions_module.api.permission');
    
        // create query
        $orderBy = $this->container->get('rk_almanac_module.model_helper')->resolveSortParameter($this->objectType, $this->sorting);
        ...
        $templateParameters = $this->container->get('rk_almanac_module.controller_helper')->addTemplateParameters($this->objectType, $templateParameters, 'contentType', []);
    ...
    protected function getDisplayTemplate()
    {
        ...
        foreach ($templateOptions as $templatePath) {
            if ($templating->exists('@RKAlmanacModule/' . $templatePath)) {
                $template = '@RKAlmanacModule/' . $templatePath;
                break;
            }
        }
    ...
    public function startEditing()
    {
        ...
        $this->view->toplevelmodule = 'RKAlmanacModule';
    
        // ensure our custom plugins are loaded
        array_push($this->view->plugins_dir, 'modules/RK/AlmanacModule/Resources/views/plugins');
        ...
        if ($featureActivationHelper->isEnabled(FeatureActivationHelper::CATEGORIES, $this->objectType)) {
            ...
            $this->view->assign('categories', $categories)
                       ->assign('categoryHelper', $this->container->get('rk_almanac_module.category_helper'));
        }
