...
abstract class AbstractSearchHelper implements SearchableInterface
{
    ...
    {
        if (!$this->permissionApi->hasPermission('RKAlmanacModule::', '::', ACCESS_READ)) {
            return '';
        }
    ...
    public function getResults(array $words, $searchType = 'AND', $modVars = null)
    {
        ...
        $searchTypes = $this->getSearchTypes();
        $entitiesWithDisplayAction = ['date'];
        ...
        foreach ($searchTypes as $searchTypeCode => $typeInfo) {
            ...
            if (isset($moduleActivationInfo['RKAlmanacModule'])) {
                $moduleActivationInfo = $moduleActivationInfo['RKAlmanacModule'];
            ...
            switch ($objectType) {
                case 'date':
                    ...
                    $whereArray[] = 'tbl.dateTitle';
                    $whereArray[] = 'tbl.dateDescription';
                    $whereArray[] = 'tbl.dateImage';
                    $whereArray[] = 'tbl.dateUrl';
                    break;
            }
            ...
            foreach ($entities as $entity) {
                ...
                if (!$this->permissionApi->hasPermission('RKAlmanacModule:' . ucfirst($objectType) . ':', $entity->getKey() . '::', ACCESS_OVERVIEW)) {
                    continue;
                }
                ...
                if ($hasDisplayAction) {
                    ...
                    $displayUrl = new RouteUrl('rkalmanacmodule_' . strtolower($objectType) . '_display', $urlArgs);
                }
    
                $result = new SearchResultEntity();
                $result->setTitle($formattedTitle)
                    ->setText($description)
                    ->setModule('RKAlmanacModule')
                    ->setCreated($created)
