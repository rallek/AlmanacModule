...
abstract class AbstractUploadHelper
{
    ...
    public function performFileUpload($objectType, $file, $fieldName)
    {
        ...
        try {
            ...
            $this->logger->error('{app}: User {user} could not detect upload destination path for entity {entity} and field {field}. ' . $exception->getMessage(), ['app' => 'RKAlmanacModule', 'user' => $this->currentUserApi->get('uname'), 'entity' => $objectType, 'field' => $fieldName]);
        ...
        if ($isImage) {
            ...
            if (!is_array($imgInfo) || !$imgInfo[0] || !$imgInfo[1]) {
                ...
                $this->logger->error('{app}: User {user} tried to upload a file which is seems not to be a valid image.', ['app' => 'RKAlmanacModule', 'user' => $this->currentUserApi->get('uname')]);
    ...
    protected function validateFileUpload($objectType, $file, $fieldName)
    {
        ...
        if ($file->getError() != UPLOAD_ERR_OK) {
            ...
            $this->logger->error('{app}: User {user} tried to upload a file with errors: ' . $file->getErrorMessage(), ['app' => 'RKAlmanacModule', 'user' => $this->currentUserApi->get('uname')]);
        ...
        if (false === $isValidExtension) {
            ...
            $this->logger->error('{app}: User {user} tried to upload a file with a forbidden extension ("{extension}").', ['app' => 'RKAlmanacModule', 'user' => $this->currentUserApi->get('uname'), 'extension' => $extension]);
    ...
    {
        $basePath = $this->dataDirectory . '/RKAlmanacModule/';
    
        switch ($objectType) {
            case 'date':
                $basePath .= 'dates/dateimage/';
                break;
            default:
    ...
    public function checkAndCreateAllUploadFolders()
    {
        ...
        $result &= $this->checkAndCreateUploadFolder('date', 'dateImage', 'gif, jpeg, jpg, png');
    ...
    protected function checkAndCreateUploadFolder($objectType, $fieldName, $allowedExtensions = '')
    {
        ...
        if (!$this->filesystem->exists($uploadPath)) {
            try {
                ...
                $this->logger->error('{app}: The upload directory {directory} does not exist and could not be created.', ['app' => 'RKAlmanacModule', 'directory' => $uploadPath]);
        ...
        if (!is_writable($uploadPath)) {
            try {
                ...
                $this->logger->error('{app}: The upload directory {directory} exists but is not writable by the webserver.', ['app' => 'RKAlmanacModule', 'directory' => $uploadPath]);
        ...
        $htaccessFileTemplate = 'modules/RK/AlmanacModule/Resources/docs/htaccessTemplate';
        if (!$this->filesystem->exists($htaccessFilePath) && $this->filesystem->exists($htaccessFileTemplate)) {
            try {
                ...
                $this->logger->error('{app}: An error occured during creation of the .htaccess file in directory {directory}.', ['app' => 'RKAlmanacModule', 'directory' => $uploadPath]);
