...
use RK\AlmanacModule\Helper\ArchiveHelper;
use RK\AlmanacModule\Helper\CollectionFilterHelper;
...
abstract class AbstractControllerHelper
{
    ...
    /**
     * Returns an array of all allowed object types in RKAlmanacModule.
     *
    ...
    public function getObjectTypes($context = '', array $args = [])
    {
        ...
        $allowedObjectTypes = [];
        $allowedObjectTypes[] = 'date';
    
        return $allowedObjectTypes;
    }

    /**
     * Returns the default object type in RKAlmanacModule.
     *
     * @param string $context Usage context (allowed values: controllerAction, api, helper, actionHandler, block, contentType, util)
     * @param array  $args    Additional arguments
     *
    ...
    public function processViewActionParameters($objectType, SortableColumns $sortableColumns, array $templateParameters = [], $hasHookSubscriber = false)
    {
        ...
        $templateParameters['own'] = $request->query->getInt('own', $this->variableApi->get('RKAlmanacModule', 'showOnlyOwnEntries', 0));
        ...
        if ($templateParameters['all'] != 1) {
            ...
            if (in_array($resultsPerPage, [0, 10])) {
                $resultsPerPage = $this->variableApi->get('RKAlmanacModule', $objectType . 'EntriesPerPage', 10);
            }
        }
        ...
        $quickNavForm = $this->formFactory->create('RK\AlmanacModule\Form\Type\QuickNavigation\\' . ucfirst($objectType) . 'QuickNavType', $templateParameters);
        ...
        if (true === $hasHookSubscriber) {
            ...
            $templateParameters['currentUrlObject'] = new RouteUrl('rkalmanacmodule_' . strtolower($objectType) . '_view', $urlParameters);
        }
    ...
    public function processDisplayActionParameters($objectType, array $templateParameters = [], $hasHookSubscriber = false)
    {
        ...
        if (true === $hasHookSubscriber) {
            ...
            $templateParameters['currentUrlObject'] = new RouteUrl('rkalmanacmodule_' . strtolower($objectType) . '_display', $urlParameters);
        }
    
        if (in_array($objectType, ['date'])) {
            $qb = $this->entityFactory->getObjectManager()->createQueryBuilder();
            $qb->select('tbl')
               ->from('RK\AlmanacModule\Entity\HookAssignmentEntity', 'tbl')
    ...
    public function addTemplateParameters($objectType = '', array $parameters = [], $context = '', array $args = [])
    {
        ...
        if ($context == 'controllerAction') {
            ...
            if ($objectType == 'date') {
                $thumbRuntimeOptions = [];
                $thumbRuntimeOptions[$objectType . 'DateImage'] = $this->imageHelper->getRuntimeOptions($objectType, 'dateImage', $context, $args);
            ...
            if (in_array($args['action'], ['display', 'edit', 'view'])) {
                ...
                $parameters['relationThumbRuntimeOptions'] = $this->imageHelper->getCustomRuntimeOptions('', '', 'RKAlmanacModule_relateditem', $context, $args);
            }
        }
