...
abstract class AbstractArchiveHelper
{
    ...
    public function archiveObsoleteObjects($probabilityPercent = 75)
    {
        ...
        if (!$this->permissionApi->hasPermission('RKAlmanacModule', '.*', ACCESS_EDIT)) {
            // abort if current user has no permission for executing the archive workflow action
            return;
        }
    
        // perform update for dates becoming archived
        $logArgs = ['app' => 'RKAlmanacModule', 'entity' => 'date'];
    ...
    /**
     * Moves dates into the archive which reached their end date.
     *
     * @throws RuntimeException Thrown if workflow action execution fails
     */
    protected function archiveDates()
    {
        $today = date('Y-m-d H:i:s');
    
        $affectedEntities = $this->getObjectsToBeArchived('date', 'endDate', $today);
    ...
    protected function archiveSingleObject($entity)
    {
        ...
        if ($entity->supportsHookSubscribers()) {
            ...
            $hasDisplayPage = in_array($objectType, ['date']);
            if ($hasDisplayPage) {
                $urlArgs = $entity->createUrlArgs();
                if (null !== $request) {
                    $urlArgs['_locale'] = $request->getLocale();
                }
                $url = new RouteUrl('rkalmanacmodule_' . strtolower($objectType) . '_display', $urlArgs);
        	}
