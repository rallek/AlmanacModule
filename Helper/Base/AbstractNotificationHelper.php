...
abstract class AbstractNotificationHelper
{
    ...
    protected function collectRecipients($debug = false)
    {
        ...
        if ($this->recipientType == 'moderator' || $this->recipientType == 'superModerator') {
            ...
            $moderatorGroupId = $this->variableApi->get('RKAlmanacModule', 'moderationGroupFor' . $modVarSuffix, GroupsConstant::GROUP_ID_ADMIN);
            if ($this->recipientType == 'superModerator') {
                $moderatorGroupId = $this->variableApi->get('RKAlmanacModule', 'superModerationGroupFor' . $modVarSuffix, GroupsConstant::GROUP_ID_ADMIN);
            }
    ...
    protected function sendMails()
    {
        ...
        foreach ($this->recipients as $recipient) {
            ...
            $body = $this->templating->render('@RKAlmanacModule/' . $template, [
                'recipient' => $recipient,
                'mailData' => $mailData
            ]);
            $altBody = '';
    ...
    protected function prepareEmailData()
    {
        ...
        $hasDisplayAction = in_array($objectType, ['date']);
        $hasEditAction = in_array($objectType, ['date']);
        $routeArea = in_array($this->recipientType, ['moderator', 'superModerator']) ? 'admin' : '';
        $routePrefix = 'rkalmanacmodule_' . strtolower($objectType) . '_' . $routeArea;
