...
use RK\AlmanacModule\Helper\CategoryHelper;
...
abstract class AbstractCollectionFilterHelper
{
    ...
    public function getViewQuickNavParameters($objectType = '', $context = '', array $args = [])
    {
        ...
        if ($objectType == 'date') {
            return $this->getViewQuickNavParametersForDate($context, $args);
        }
    ...
    {
        if ($objectType == 'date') {
            return $this->addCommonViewFiltersForDate($qb);
        }
    ...
    {
        if ($objectType == 'date') {
            return $this->applyDefaultFiltersForDate($qb, $parameters);
        }
     ...
     * @return array List of template variables to be assigned
     */
    protected function getViewQuickNavParametersForDate($context = '', array $args = [])
    {
        $parameters = [];
     ...
     * @return QueryBuilder Enriched query builder instance
     */
    protected function applyDefaultFiltersForDate(QueryBuilder $qb, array $parameters = [])
    {
        if (null === $this->request) {
            return $qb;
        }
        $routeName = $this->request->get('_route');
        $isAdminArea = false !== strpos($routeName, 'rkalmanacmodule_date_admin');
        if ($isAdminArea) {
            return $qb;
        }
        ...
        if (!in_array('workflowState', array_keys($parameters)) || empty($parameters['workflowState'])) {
            // per default we show approved dates only
            $onlineStates = ['approved'];
            if ($showOnlyOwnEntries) {
                // allow the owner to see his dates
        ...
        $qb = $this->applyDateRangeFilterForDate($qb);
    
        return $qb;
    }
    
    /**
     * Applies start and end date filters for selecting dates.
     *
     * @param QueryBuilder $qb    Query builder to be enhanced
    ...
    protected function applyDateRangeFilterForDate(QueryBuilder $qb, $alias = 'tbl')
    {
        ...
        $qb->andWhere($alias . '.startDate <= :startDate')
    ...
    public function addSearchFilter($objectType, QueryBuilder $qb, $fragment = '')
    {
        ...
        if ($objectType == 'date') {
            ...
            $filters[] = 'tbl.dateTitle LIKE :searchDateTitle';
            ...
            $filters[] = 'tbl.dateImage = :searchDateImage';
            $parameters['searchDateImage'] = $fragment;
            $filters[] = 'tbl.dateUrl = :searchDateUrl';
            $parameters['searchDateUrl'] = $fragment;
        }
