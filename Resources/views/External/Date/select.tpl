...
<div class="row">
    <div class="col-sm-8">
        ...
        {if $properties ne null && is_array($properties)}
            ...
            {foreach key='propertyName' item='propertyId' from=$properties}
                <div class="form-group">
                    {assign var='hasMultiSelection' value=$categoryHelper->hasMultipleSelection('date', $propertyName)}
        ...
        {/if}
        <div class="form-group">
            <label for="{$baseID}Id" class="col-sm-3 control-label">{gt text='Date'}:</label>
            <div class="col-sm-9">
                <select id="{$baseID}Id" name="id" class="form-control">
                    {foreach item='date' from=$items}
                        <option value="{$date->getKey()}"{if $selectedId eq $date->getKey()} selected="selected"{/if}>{$date->getDateTitle()}</option>
                    {foreachelse}
        ...
        <div class="form-group">
            ...
            <div class="col-sm-9">
                <select id="{$baseID}Sort" name="sort" class="form-control">
                    ...
                    <option value="dateUrl"{if $sort eq 'dateUrl'} selected="selected"{/if}>{gt text='Date url'}</option>
            ...
            <div class="col-sm-9">
                <div class="input-group">
                    ...
                    <span class="input-group-btn">
                        <input type="button" id="rKAlmanacModuleSearchGo" name="gosearch" value="{gt text='Filter'}" class="btn btn-default" />
                    </span>
                </div>
            </div>
        </div>
    ...
    <div class="col-sm-4">
        <div id="{$baseID}Preview" style="border: 1px dotted #a3a3a3; padding: .2em .5em">
            <p><strong>{gt text='Date information'}</strong></p>
