 ...
 */
function rKAlmanacResetUploadField(fieldName) {
 ...
 */
function rKAlmanacInitUploadField(fieldName) {
    jQuery('#' + fieldName + 'ResetVal').click(function (event) {
        event.preventDefault();
        rKAlmanacResetUploadField(fieldName);
    }).removeClass('hidden');
}
 ...
 */
function rKAlmanacInitDateField(fieldName) {
    jQuery('#' + fieldName + 'ResetVal').click(function (event) {
        event.preventDefault();
...
function rKAlmanacTriggerFormValidation() {
    rKAlmanacExecuteCustomValidationConstraints(editedObjectType, editedEntityId);
...
function rKAlmanacHandleFormSubmit (event) {
    if (triggerValidation) {
        rKAlmanacTriggerFormValidation();
        if (!editForm.get(0).checkValidity()) {
            event.preventDefault();
            return false;
        }
 ...
 */
function rKAlmanacInitEditForm(mode, entityId) {
    if (jQuery('.rkalmanac-edit-form').length < 1) {
        return;
    }

    editForm = jQuery('.rkalmanac-edit-form').first();
    ...
    editForm.submit(rKAlmanacHandleFormSubmit);
 ...
 */
function rKAlmanacInitRelationHandling(objectType, alias, idPrefix, includeEditing, inputType, createUrl) {
    if (inputType == 'autocomplete') {
        rKAlmanacInitAutoCompletion(objectType, alias, idPrefix, includeEditing);
    }
    if (includeEditing) {
        rKAlmanacInitInlineEditingButtons(objectType, alias, idPrefix, inputType, createUrl);
    }
}
