 ...
 */
function rKAlmanacToggleAutoCompletionFields(idPrefix) {
 ...
 */
function rKAlmanacResetAutoCompletion(idPrefix) {
    // hide the sub form
    rKAlmanacToggleAutoCompletionFields(idPrefix);
 ...
 */
function rKAlmanacRemoveRelatedItem(idPrefix, removeId) {
 ...
 */
function rKAlmanacSelectResultItem(objectType, idPrefix, selectedListItem, includeEditing) {
    ...
    if (true === includeEditing) {
        li.append(rKAlmanacCreateInlineEditLink(objectType, idPrefix, elemPrefix, newItemId));
    }

    li.append(
        jQuery('<a />', {
            id: elemPrefix + 'Remove',
            href: 'javascript:rKAlmanacRemoveRelatedItem(\'' + idPrefix + '\', ' + newItemId + ');',
            text: 'remove'
        }).append(
    ...
    if (true === includeEditing) {
        rKAlmanacInitInlineEditLink(objectType, idPrefix, elemPrefix, newItemId, 'autocomplete');
    }

    itemIds += newItemId;
    jQuery('#' + idPrefix).val(itemIds);

    rKAlmanacResetAutoCompletion(idPrefix);
}

/**
 * Adds a hook assignment item to selection which has been chosen by auto completion.
 */
function rKAlmanacSelectHookItem(objectType, idPrefix, selectedListItem) {
    rKAlmanacResetAutoCompletion(idPrefix);
    rKAlmanacAttachHookObject(jQuery('#' + idPrefix + 'AddLink'), selectedListItem.id);
}

/**
 * Initialises auto completion for a relation field.
 */
function rKAlmanacInitAutoCompletion(objectType, alias, idPrefix, includeEditing) {
        ...
        jQuery('#' + idPrefix + 'Selector').autocomplete({
            ...
            select: function (event, ui) {
                if (true === isHookAttacher) {
                    rKAlmanacSelectHookItem(objectType, idPrefix, ui.item);
                } else {
                    rKAlmanacSelectResultItem(objectType, idPrefix, ui.item, includeEditing);
                }
