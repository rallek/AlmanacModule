...
abstract class AbstractAjaxController extends AbstractController
{
    ...
    {
        if (!$this->hasPermission('RKAlmanacModule::Ajax', '::', ACCESS_EDIT)) {
            return true;
        }
        
        $objectType = $request->query->getAlnum('ot', 'date');
        ...
        $entityDisplayHelper = $this->get('rk_almanac_module.entity_display_helper');
    ...
    protected function prepareSlimItem($repository, $objectType, $item, $itemId, $descriptionField)
    {
        ...
        $previewParameters = $this->get('rk_almanac_module.controller_helper')->addTemplateParameters($objectType, $previewParameters, 'controllerAction', $contextArgs);
    
        $previewInfo = base64_encode($this->get('twig')->render('@RKAlmanacModule/External/' . ucfirst($objectType) . '/info.html.twig', $previewParameters));
    
        $title = $this->get('rk_almanac_module.entity_display_helper')->getFormattedTitle($item);
    ...
    public function getItemListAutoCompletionAction(Request $request)
    {
        ...
        if ((is_array($entities) || is_object($entities)) && count($entities) > 0) {
            ...
            $imageHelper = $this->get('rk_almanac_module.image_helper');
    ...
    public function attachHookObjectAction(Request $request)
    {
        ...
        $assignment = new \RK\AlmanacModule\Entity\HookAssignmentEntity();
        ...
        $entityManager = $this->get('rk_almanac_module.entity_factory')->getObjectManager();
    ...
    public function detachHookObjectAction(Request $request)
    {
        ...
        $qb->delete('RK\AlmanacModule\Entity\HookAssignmentEntity', 'tbl')
