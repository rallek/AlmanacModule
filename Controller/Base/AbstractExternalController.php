...
abstract class AbstractExternalController extends AbstractController
{
    ...
    public function displayAction(Request $request, $objectType, $id, $source, $displayMode)
    {
        ...
        $component = 'RKAlmanacModule:' . ucfirst($objectType) . ':';
        if (!$this->hasPermission($component, $id . '::', ACCESS_READ)) {
            return '';
        }
        
        $entityFactory = $this->get('rk_almanac_module.entity_factory');
        ...
        $contextArgs = ['controller' => 'external', 'action' => 'display'];
        $templateParameters = $this->get('rk_almanac_module.controller_helper')->addTemplateParameters($objectType, $templateParameters, 'controllerAction', $contextArgs);
        
        return $this->render('@RKAlmanacModule/External/' . ucfirst($objectType) . '/' . $template, $templateParameters);
    }
    ...
    public function finderAction(Request $request, $objectType, $editor, $sort, $sortdir, $pos = 1, $num = 0)
    {
        ...
        $cssAssetBag->add($assetHelper->resolve('@RKAlmanacModule:css/style.css'));
        $cssAssetBag->add([$assetHelper->resolve('@RKAlmanacModule:css/custom.css') => 120]);
        ...
        if (!in_array($objectType, $activatedObjectTypes)) {
            ...
            $redirectUrl = $this->get('router')->generate('rkalmanacmodule_external_finder', ['objectType' => array_shift($activatedObjectTypes), 'editor' => $editor]);
        
            return new RedirectResponse($redirectUrl);
        }
        
        if (!$this->hasPermission('RKAlmanacModule:' . ucfirst($objectType) . ':', '::', ACCESS_COMMENT)) {
            throw new AccessDeniedException();
        }
        ...
        $form = $this->createForm('RK\AlmanacModule\Form\Type\Finder\\' . ucfirst($objectType) . 'FinderType', $templateParameters, $formOptions);
        ...
        if ($searchTerm != '') {
            $qb = $this->get('rk_almanac_module.collection_filter_helper')->addSearchFilter($objectType, $qb, $searchTerm);
        }
        ...
        if (in_array($objectType, ['date'])) {
            ...
            if ($featureActivationHelper->isEnabled(FeatureActivationHelper::CATEGORIES, $objectType)) {
                $entities = $this->get('rk_almanac_module.category_helper')->filterEntitiesByPermission($entities);
            }
        }
        ...
        $output = $this->renderView('@RKAlmanacModule/External/' . ucfirst($objectType) . '/find.html.twig', $templateParameters);
